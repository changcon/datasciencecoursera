...
```{r Pre-processing the Activity Dataset}
...
The mean number of steps is `r meanSteps` steps while the median number of steps is `r medianSteps`.
...
```
The interval with the max number of steps is `r IntervalMax$interval`.
...
medianSteps=median(stepsDate$dailySteps)
```
The mean number of steps is `r meanSteps` while the median number of steps is `r medianSteps`. The median and mean are now the same whereas before the median was slightly less. Imputing the mean per interval value for missing steps caused the median and mean values to align.
